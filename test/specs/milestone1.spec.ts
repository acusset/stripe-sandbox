...
import { TWENTY_SECONDS, TYPE_DELAY, VALID_CARD, checkTitle, fillCardDetails, fillPersonalDetails, lessonSignUp, makeDateString, openRegistrationPane, serverRequest, stripeRequest, submitForm, test } from '../helpers';
...
test.describe('Using different test cards', () => {
  ...
  test('Should schedule a Lesson using a 3DS Card:3.22', async ({ page }) => {
    ...
    await page.frame({name: 'stripe-challenge-frame'})?.locator('#test-source-authorize-3ds').click({ timeout: 30 * 1000 });
